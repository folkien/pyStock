...
#             self.buy  = fromTop
        ...
        # Plot method
        def Plot(self):
            plt.plot(self.jaw.index, self.jaw, "#0000FF", label="Jaw", linewidth=.8)
            plt.plot(self.teeth.index, self.teeth, "#FF0000", label="Teeth", linewidth=.8)
            plt.plot(self.lips.index, self.lips, "#00FF00", label="Lips", linewidth=.8)
            # Signals plottting
