...
def PlotAsset(ax, xpos, asset):
    if (asset['operation'] == 'buy'):
        ax.plot(xpos, asset['price'], 'o', color='k', ms=10)
        ax.plot(xpos, asset['price'], 'o', color='w', ms=8)
        ax.plot(xpos, asset['price'], 'v', color='g', label='Asset in')
    else:
        ax.plot(xpos, asset['price'], 's', color='k', ms=10)
        ax.plot(xpos, asset['price'], 's', color='w', ms=8)
        ax.plot(xpos, asset['price'], '^', color='r', label='Asset out')
...
def ReportAsset(file, asset, currentClosePrice, currencySymbol):
    ...
    if (change >= 0):
        file.write("<span style='color:green'>%d%s +%2.2f%% +%d%s </span> " %
                   (currentValue, currencySymbol, change, valueDelta, currencySymbol))
    else:
        file.write("<span style='color:red'>**%d%s** %2.2f%% %d%s </span> " %
                   (currentValue, currencySymbol, change, valueDelta, currencySymbol))
    file.write(' from **%d%s** (%dj x %d%s) \n' % (originalValue,
                                                   currencySymbol, asset['number'], asset['price'], currencySymbol))
...
class Asset(object):
    ...
    def Report(self, file, currencySymbol):
        ...
        # Income
        if (self.change >= 0):
            file.write("<span style='color:green'>+%2.2f%% +%2.2f%s</span> " %
                       (self.change, self.income, currencySymbol))
        else:
            file.write("<span style='color:red'>%2.2f%% %2.2f%s</span> " %
                       (self.change, self.income, currencySymbol))
        # Current value
        file.write('**%d%s** (%dj x %2.2f%s) ' % (self.currentValue,
                                                  currencySymbol, self.data['number'], self.currentPrice, currencySymbol))
        # Original value
        file.write('from **%d%s** (%dj x %2.2f%s) \n' % (self.originalValue,
                                                         currencySymbol, self.data['number'], self.data['price'], currencySymbol))
...
class StockAssets(object):
    ...
    def Report(self, file, currencySymbol):
        ...
        if (len(self.assets) > 0):
            ...
            file.write('**%d%s** from **%d%s**.\n' % (totalValue,
                                                      currencySymbol, totalInvested, currencySymbol))
            file.write('\n')
    ...
    def ReportForCode(self, stockCode, file, currencySymbol):
        ...
        if (len(self.assets) > 0):
            ...
            if (income >= 0):
                file.write(
                    "<span style='color:green'>+%2.2f%%</span>" % (change))
            else:
            ...
            if (income >= 0):
                ...
                file.write("<span style='color:red'>%d%s %d%s</span> from %d%s.\n" % (income,
                                                                                      currencySymbol, totalValue, currencySymbol, totalInvested, currencySymbol))
            file.write('\n')
